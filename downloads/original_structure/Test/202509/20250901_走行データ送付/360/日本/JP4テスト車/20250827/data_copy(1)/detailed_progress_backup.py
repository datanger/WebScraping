...
class DetailedProgressTracker:
    ...
    def _display_progress(self):
        ...
        with self.lock:
            ...
            try:
                ...
                print(f"Overall Progress: {completed_tasks}/{total_tasks} ({overall_progress:.1f}%) | Time: {elapsed_time}")
                print(f"Running: {running_tasks} | Completed: {completed_tasks} | Failed: {failed_tasks}")
                print("-"*60)
    ...
    def _display_task_progress(self, task_id: str, task: Dict[str, Any]):
        ...
        # 显示格式，包含文件数量信息
        files_info = f"({copied_files}/{total_files})" if total_files > 0 else "(0/0)"
        print(f"{status_icon} {task_id:<25} [{bar}] {progress:5.1f}% {files_info}")
